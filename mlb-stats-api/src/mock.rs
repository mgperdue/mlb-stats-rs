...
/// | `get_meta` | `"meta"` |
///
/// Note: the raw `get::<T>()` escape hatch is intentionally not mocked here.
/// Use the real client or construct a wiremock server for raw endpoint testing.
///
/// # Example
///
/// ```rust
...
impl MockMlbClient {
    ...
    pub async fn get_live_game_diff_patch(
        ...
        if let Some(end) = end_time_code {
            params.push(format!("endTimecode={end}"));
        }
        self.resolve("live_game_diff_patch", params)
    }
    ...
    pub async fn get_person(
        ...
        let mut params = vec![format!("playerId={player_id}")];
        if let Some(h) = hydrate {
            params.push(format!("hydrate={h}"));
        }
        self.resolve("person", params)
    }
    ...
    pub async fn get_game_officials(
        ...
        self.resolve("game_officials", vec![format!("gamePk={game_pk}")])
    }
    ...
    pub async fn get_venues(
        ...
        let mut params = vec![format!("venueId={venue_id}")];
        if let Some(h) = hydrate {
            params.push(format!("hydrate={h}"));
        }
        self.resolve("venues", params)
    }
